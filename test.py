...
class JiraChatbot:
    ...
    def _get_priority_map(self) -> Dict[str, str]:
        ...
        try:
            ...
            if not priority_map:
                logger.warning("No priorities fetched from JIRA, using default enum values")
                return {p.value: p.value for p in Priority}
    ...
    async def _validate_ticket_request(self, ticket_request: JiraTicketRequest) -> None:
        ...
        if priority_name not in available_priorities:
            available_str = ', '.join(available_priorities)
            raise ValueError(f"Priority '{ticket_request.priority.value}' not available in JIRA. Available: {available_str}")
