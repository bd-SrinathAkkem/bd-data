...
- Validates custom priorities: ['Pe Urgent', 'P1 Current priority', ...].
- Adds tickets to the active sprint (requires board_id or auto-fetches).
- AI enhancement with Ollama, Claude, or custom providers.
- Teams notifications with Adaptive Cards.
- API key authentication (X-API-Key) and CORS.
- Jira authentication via JIRA_API_TOKEN.
- Structured logging with debug info for API keys.
- No Redis dependency.
...
- API_KEY: Required API key for FastAPI
- ALLOWED_ORIGINS: Comma-separated CORS origins (default: *)
- AI_PROVIDER_DEFAULT: Default AI provider (ollama/claude/custom)

Run Locally:
uvicorn jira_chatbot:app --reload

Production Run:
gunicorn -k uvicorn.workers.UvicornWorker -w 4 jira_chatbot:app

Direct Execution:
python3 jira_chatbot.py
...
def jira_connection(config: JiraConfig):
    try:
        ...
        logger.info({"action": "jira_connection_established"})
        yield jira
    except JIRAError as e:
        logger.error({"error": str(e), "type": "JiraConnectionError"})
        raise HTTPException(status_code=500, detail=f"Jira connection error: {str(e)}. Verify JIRA_API_TOKEN and JIRA_USERNAME.")
    finally:
        pass
...
async def create_ticket(
    ...
    """
    logger.info({"action": "create_ticket_start", "client_ip": request.client.host, "timestamp": "2025-09-01 16:07:00"})
